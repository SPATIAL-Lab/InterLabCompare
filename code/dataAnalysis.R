...
il1 <- read.csv('data/intralab1.csv') 
...
intralabO <- ggplot() + 
  geom_hline(yintercept = 0, color = 'grey20', linetype = 2) +
  geom_boxplot(data = subset(il, treatment != "Treated, 30 Rxn Temp" & treatment != "Untreated, 30 Rxn Temp"), 
...
ggarrange(intralabO, intralabC, nrow = 1)
...
ggplot() + 
  geom_point(data = il1, aes(x = d13Cuu, y = d13Cdpaa, color = treatment), size = 3) + 
...
ggsave("figures/intrascatterC.png", units = c("in"), width = 7, height = 5)

ggplot() + 
  geom_point(data = il1, aes(x = d18Ouu, y = d18Odpaa, color = treatment), size = 3) + 
  geom_abline(slope=1, intercept = 0) +
  scale_color_manual(values = cols3, 
                     name = "Treatment") + 
  theme_classic() +
  ...
  guides(color = guide_legend(nrow = 2))
ggsave("figures/intrascatterO.png", units = c("in"), width = 7, height = 5)
...
O4 <- ggplot() + 
  ...
  labs(x = expression(paste('UU ', delta^18, 'O', " (\u2030)")), 
       y = expression(paste('DPAA ', delta^18, 'O', " (\u2030)")))
ggarrange(O1, O2, O3, O4, nrow = 2, ncol = 2)
